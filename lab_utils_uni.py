""" 
lab_utils_uni.py
Fuente original de las rutinas: https://www.coursera.org/learn/machine-learning
"""
import numpy as np
...
def plt_house_x(X, y,f_wb=None, ax=None):
    ...
    ax.set_title("Valor medio de las viviendas")
    ax.set_ylabel('Precio medio viviendas (en 1000s de dolares)')
    ax.set_xlabel(f'Ingresos medios (en 10k dolares)')
    if f_wb is not None:
        ax.plot(X, f_wb,  c=dlblue, label="Predicción")
    ax.legend()
...
def plt_intuition(x_train, y_train, fixed_b):
    ...
    def func( w=150):
        ...
        ax[1].scatter(w,cur_cost, s=100, color=dldarkred, zorder= 10, label= f"Coste con w={w}")
        ...
        ax[1].set_title(f"Coste vs. w, (b fijado a {tmp_b})")
        ax[1].set_ylabel('Coste')
        ax[1].set_xlabel('w')
        ax[1].legend(loc='upper center')
        fig.suptitle(f"Minimizar coste: Coste actual = {cur_cost:0.0f}", fontsize=12)
        plt.show()
...
def plt_stationary(x_train, y_train):
    ...
    plt.title("Coste(w,b) \n [Puedes rotar la figura]", size=12)
...
def soup_bowl():
    ...
    ax.set_title("$J(w,b)$\n [Puedes rotar la figura]", size=15)
...
def plt_contour_wgrad(x, y, hist, ax, w_range=[-100, 500, 5], b_range=[-500, 500, 5],
    ...
    ax.set_title('Coste J(w,b), vs b,w a través de la evolución del gradiente descendente')
    w = w_final; b=b_final
...
def plt_gradients(x_train,y_train, f_compute_cost, f_compute_gradient, fixed_b=100, tested_points=[100,200,300]):
    #===============
    #  First subplot
    #===============
    ...
    ax.set_title(f"Coste vs w, con gradient; b fijado a {fix_b}")
    ax.set_ylabel('Coste')
    ax.set_xlabel('w')
    
    # plot lines for fixed b=100
    for tmp_w in tested_points:
